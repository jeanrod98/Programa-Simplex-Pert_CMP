    ...
    def __init__(self, ui, icono):
     ...
     #║Icono de las alertas
      ...
      #Eventos metodo Pert
        ...
        self.ui.imprimir_pert.clicked.connect(self.generarReporte)
        self.ui.imprimir_pert.setEnabled(False)
        self.btn_nuevo()
...
##Alerta de Validacion
    ...
    def generar_tabla_2(self, array_Dij):
        ...
        for m in range(int(self.num_actividad)):
            ...
            #Colocamos el valor de MTij y colocamos color a las filas          
            
            if Mtij == 0:
                self.ui.tabla2.setItem(m,6,QTableWidgetItem(str(Mtij)))
                for c in range(7):
                    self.ui.tabla2.item(m,c).setBackground(QtGui.QColor(233,42,0))
    ...
    def generar_fechas(self):
        self.ui.imprimir_pert.setEnabled(True)
        # self.sacar_actividades_criticas()
        #Validar si se trabajan sabado o domingo
        self.sabado = self.ui.sabado_2.isChecked()
        self.domingo = self.ui.domingo_2.isChecked() 
    ...
    def comprobar_fecha_2(self, fechas):
        ...
        return fecha2
    #funcion que pinta las celdas criticas
    def color_celdas(self, posicion):
        #recorrer las filas
        
        for f in range(int(self.num_actividad)):
            #recorremos las columnas
            for c in range(11):
                
                self.ui.tabla2.item(posicion,c).setBackground(QtGui.QColor(233,42,0))

    #generar reporte del resultado
    def generarReporte(self):
        ...
        text, ok = QInputDialog.getText(self, 'Generar Reporte',
                                        'Ingrese el nombre de quien genera el reporte:')
        ...
        if ok:
            ...
            try:
                ...
                if self.sabado == True and self.domingo == True:
                    self.dia_trabajo = "Los días que no se trabajarán son Sabado y Domingo."
                elif self.sabado == False and self.domingo == False:
                    self.dia_trabajo = "Se trabajan todos los dias de la semana."

                #Array para capturar datos de la tabla 2
                Filas = []               

                for i in range(int(self.num_actividad)):
                    array_filas = []
                    for j in range(11):
                        
                        item = self.ui.tabla2.item(i,j,).text()
                        array_filas.append(item)
                    Filas.append(array_filas)
                ...
                for i in range(int(self.num_actividad)):
                    ...
                    for j in range(5):
                        ...
                        item = self.ui.tabla1.item(i,j,).text()
                        array_filas.append(item)
                    Filas_t1.append(array_filas)
            

                doc = SimpleDocTemplate("Reporte_pert.pdf", pagesize = landscape(A4), topMargin=12)
                alineacionTitulo = ParagraphStyle(name="centrar", alignment=TA_CENTER, fontSize=20, leading=40)
                alineacionProyecto = ParagraphStyle(name="centrar", alignment=TA_CENTER, fontSize=14, leading=40)
                alineacionAutor = ParagraphStyle(name="centrar", alignment=TA_CENTER, fontSize=11, leading=40)
                alineacionResultados = ParagraphStyle(name="centrar", alignment=TA_LEFT, fontSize=12, leading=60)
                alineacion_R_critica = ParagraphStyle(name="centrar", alignment=TA_CENTER, fontSize=12, leading=40)
                # tablas = ParagraphStyle(name="centrar",leading=30)
                # alineacionFuncion = ParagraphStyle(name="centrar", alignment=TA_LEFT, fontSize=12, leading=40)
                
                #Titulo para la tabla 2
                headings = ('Actividad', 'Dij', 'Ti0', 'Ti1',
                            'Tj0', 'Tj1', 'MTij', 'Inicio pronto',
                            'Inicio Tardio', 'Fin Temprano', 'Fin Tardio')
                #Titulo para la tabla 1
                headings_1 = ('Actividad', 'Predecesores', 'To', 'Tn', 'Tp')

                story=[]
                t = Table([headings]+Filas, colWidths=75, rowHeights=24)
                ...
                t1 = Table([headings_1]+Filas_t1, colWidths=75, rowHeights=24)
                ...
                rutas = self.sacar_actividades_criticas()
                titulo = "Reporte Método Pert CMP"
                autor = f'Autor:  {text}'
                nombre_archivo = resolver_ruta("logo.png")
                story.append(Image(nombre_archivo))
                ...
                story.append(Paragraph(self.dia_trabajo, alineacionResultados))
                story.append(t1)
                story.append(Paragraph(f'La fecha de inicio del proyecto es: {self.fecha_inicio}',alineacionResultados))
                story.append(Paragraph(f'Resultados del Proyecto',alineacionProyecto))
                story.append(Paragraph(f'Las rutas criticas son: {rutas}',alineacion_R_critica))
                story.append(t)               
                story.append(Spacer(0,15))
                doc.build(story)
                os.system("Reporte_pert.pdf")

            
                print("Se genero el reporte")
            except PermissionError:
                print("No se genero el PDF")
    #Sacar las actividades criticas
    def sacar_actividades_criticas(self):
        ...
        #recorremos las columnas
            for c in range(1):
                ...
                item = self.ui.tabla2.item(f,c).text()
                item_z = int(self.ui.tabla2.item(f,6).text())
                if item_z == 0:
                    if f == 0:
                        letras.append(item)
                    else:
                        letras.append(f'-{item}')
        str_letras = "".join(letras)
        return str_letras
